...
func TestHarvestOpenAICodexTicket_StopsAt292AndUsesHarvestProxy(t *testing.T) {
	...
	expectedVersion := openAICodexAstraMinVersion
	if CompareVersions(codexCLIVersion, openAICodexAstraMinVersion) >= 0 {
		expectedVersion = codexCLIVersion
	}
	require.Equal(t, expectedVersion, upstream.requests[0].Header.Get("version"))
